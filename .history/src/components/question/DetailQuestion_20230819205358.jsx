...
export default function DetailQuestion({ user, item }) {
  ...
  const [questionDatailModalEditIdBucket, setQuestionDatailModalEditIdBucket] =
    useState("");
  ...
  const handleEditQuestion = (e, idx) => {
    ...
    if (result) {
      ...
      setQuestionDatailModalEditIdBucket(idx);
    }
  };
            ...
            .map((data) => {
              return (
                <div className="p-5 box-border">
                  ...
                  questionDatailModalEditIdBucket === data.id ? (
                    <input
                      type="text"
                      placeholder="수정할 내용을 입력하세요."
                      className="w-full p-3 mt-5 mb-80 box-border border-[1px] border-solid border-[#ccc] rounded-lg outli"
                    />
                  ) : (

...
export default function Scroll({ contentRef }) {
  ...
  return (
    <div
      ...
      } flex flex-col text-[0.8rem] fixed bottom-5 right-5`}
    >
      <button
        onClick={handleTop}
        className={`${
          showButton ? "bg-opacity-100" : "bg-"
        } "w-16 h-16 bg-slate-900 rounded-full drop-shadow-[0_5px_2px_rgba(0,0,0,0.25)] text-white justify-center items-center animate-bounce`}
      >
        <p>UP</p>
      </button>
      <button

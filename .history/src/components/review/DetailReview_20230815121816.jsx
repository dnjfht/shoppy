...
export default function DetailReview({
  ...
  useEffect(() => {
    ...
    if (reviewOfMyBodyType && myBodyInfo) {
      filteredList = filteredList.filter((review) => {
        return (
          Number(myBodyInfo.myHeight) - 7 <=
            Number(review.reviewHeight) <=
            Number(myBodyInfo.myHeight) + 7 &&
          Number(myBodyInfo.myWeight) - 7 <=
            Number(review.reviewWeight) <=
            Number(myBodyInfo.myWeight) + 7 &&
          Number(myBodyInfo.myFootSize) - 10 <=
            Number(review.reviewBodyFoot) <=
            Number(myBodyInfo.myFootSize) + 10
        );
      });
    }

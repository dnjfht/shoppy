    ...
    return (
      ...
      >
        <div className="relative w-[160px] mr-3 group">
          ...
          />
          <div className="w-full h-full p-4 box-border rounded-lg absolute bottom-0 left-0 bg-black bg-opacity-70 text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100 flex flex-col items-center">
            <p>
              {user
                ? review.userId
            ...
            <p className="text-sm">{`색상 : ${review.reviewColorSatisfaction}`}</p>
            <p className="text-sm">{`사이즈 : ${review.reviewSizeSatisfaction}`}</p>
          </div>
        </div>
      </div>
    );

...
export default function Detail({ user, setAllCarts }) {
  ...
  const handlePushCarts = () => {
    (user) {// Instead of updating local cart, update the allCarts in the App component.
    setAllCarts((prevAllCarts) => [...prevAllCarts, ...cart]);
    setCart([]); // Clear the local cart after adding items to allCarts
    navigate("/carts")} else if(user === null) {

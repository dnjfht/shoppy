...
export default function MyCart({
  ...
  const deleteCart = async (idx) => {
    if (user) {
      const deletedAllCarts = allCarts.filter((c) => c.id !== idx);
      await setCartServer(user, deletedAllCarts);
    }
  ...
  const handleMinusCount = async (idx) => {
    if (user) {
      const minusAllCounts = setAllCarts((prevCart) =>
        prevCart.map((c) => {
          return c.id === idx && c.count > 0 ? { ...c, count: c.count - 1 } : c;
        })
      );

      await setCartServer(user, minusAllCounts);
    }

...
export default function MyCart({ cart, setCart }) {
  ...
  const freeDeliveryPercent = ((50000 - totalCount) / 50000) * 100;
  console.log(freeDeliveryPercent);
          ...
          {cart && cart.length > 0 && (
            <>
              ...
              <div className="w-[26%] h-72 border-[1px] border-solid border-[#e5e5e5]">
                {totalPrice < 50000 && (
                  <div className="w-full p-4 border-b-[1px] border-solid border-[#e5e5e5]">
                    <BsBox2 />
                    <p>{`무료배송 5만원까지 ${attach_won(
                      50000 - totalPrice
                    )} 남았습니다!`}</p>
                    <p>{freeDeliveryPercent}</p>
                    <div className="w-full h-[3px] bg-[#e5e5e5]">
                      <div
                        style={progressBarStyle}
                        className="h-full bg-[#f57778]"
                      />
                    </div>
                  </div>
                )}

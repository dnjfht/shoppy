...
export const attach_won = (prc) => {
  const price = prc.toString().slice(0, prc.length);
  const attachWon = `${price}원`;
  return attachWon;
};

// 가격에 세 자리마다 쉼표를 추가하는 함수
export const formatPriceWithCommas = (prc) => {
  // 숫자를 문자열로 변환하여 쉼표를 적용
  return prc.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
};
